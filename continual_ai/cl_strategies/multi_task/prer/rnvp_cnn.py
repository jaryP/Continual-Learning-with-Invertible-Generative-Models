...
class SplitGaussianize(torch.nn.Module):
    def __init__(self, input_dim, conditioning_size=0):
        ...
        self.g = Gaussianize(input_dim//2, self.out_dim * 2, cond_size=0)
...
class Gaussianize(torch.nn.Module):
    def __init__(self, input_dim, out_dim, cond_size=0):
        ...
        self.f = torch.nn.Conv2d(input_dim, out_dim, kernel_size=3, padding=1)  # computes the parameters of Gaussian
...
class Rescale(torch.nn.Module):
    def __init__(self, dim):
        ...
        self.weight = torch.nn.Parameter(torch.ones(dim, 1, 1,), requires_grad=True)
    ...
    def __init__(self, n_levels, levels_blocks, input_dim, hidden_size, n_hidden=1,
        ...
        if coupling_f is None:
            def coupling_f(ind, outd):
                ...
                # s = [torch.nn.Conv2d(ind, hs), torch.nn.ReLU(), torch.nn.BatchNorm1d(hs)]
                #
                ...
                net = torch.nn.Sequential(*[torch.nn.Conv2d(ind, hs, kernel_size=3, padding=1, bias=False),
                                            torch.nn.ReLU(),
                                            torch.nn.BatchNorm2d(hs),
                                            torch.nn.Conv2d(hs, hs, kernel_size=1, padding=0, bias=False),
                                            torch.nn.ReLU(),
                                            torch.nn.BatchNorm2d(hs),
                                            torch.nn.Conv2d(hs, outd, kernel_size=3, padding=1, bias=False), ])
        ...
        self.g = Gaussianize(last_dim, last_dim*2, conditioning_size)
