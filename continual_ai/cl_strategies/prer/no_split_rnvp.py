...
class Reverse(torch.nn.Module):
    ...
    def __init__(self, num_input):
        ...
        self.perm = torch.tensor(np.array(np.arange(0, num_input)[::-1]))
        self.inv_perm = torch.tensor(np.argsort(self.perm))
...
class RNVP(torch.nn.Module):
    def __init__(self, n_levels, levels_blocks, input_dim, hidden_size, n_hidden=1,
        ...
        for i in range(n_levels):
            ...
            level = RNVPLevel(input_dim, n_blocks=1, coupling_f=coupling_f,
        ...
        # self.g = Gaussianize(input_dim, input_dim, conditioning_size)
    ...
    def backward(self, x: torch.Tensor, y: torch.Tensor = None, device: str = 'cpu'):
        # if zs is None:
        #     if y is None:
        ...
        # z = zs[-1]
        # x, log_det = self.g.backward(torch.zeros_like(x), x, y)

        for i, m in enumerate(reversed(self._levels)):

            # z = zs[-i - 2]

            x, _log_det = m.backward(x, y=y)
